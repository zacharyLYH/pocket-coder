...
test.describe('harness installs are desired state', () => {
  ...
  test('install 2 per project → picker → named launch → rename → restart (frontend, isolated)', async ({
    ...
    try {
      ...
      await expect(page.getByRole('menuitem', { name: 'renamed' })).toBeVisible()
      await expect(page.getByRole('menuitem', { name: 'my-crasher' })).toBeVisible()
      await expect(page.getByRole('menuitem', { name: 'shared-name' })).toBeVisible()
      await gateShot(page, 'gate4-renamed')
      await page.getByRole('menuitem', { name: 'renamed' }).click({ timeout: 10_000 })
      await expect(sessionButton).toContainText('renamed')
