...
test.describe('harness installs are desired state', () => {
  ...
  test('install 2 per project → picker → named launch → rename → restart (frontend, isolated)', async ({
    ...
    try {
      ...
      const nameInput = dialog.getByPlaceholder(/Tab name/)
      ...
      await d1.getByPlaceholder(/Tab name/).fill('shared-name')
      ...
      await dHelper.getByPlaceholder(/Tab name/).fill('my-helper')
