...
//
//   1. seed two projects, inject a harness into ONE of them via the picker
...
// Blank sandboxes all get the derived name "untitled"; identity is the id.
...
// Row order in the pickers follows GET /api/projects (sorted by name — all
// blank sandboxes are "untitled", so ties resolve unpredictably). Always map
// ids to rows through the same API the UI renders from.
...
test.describe('harness injection orchestration', () => {
  ...
  test('injects only into selected projects; new projects opt in later', async ({ page }) => {
    test.setTimeout(180_000) // several real sandbox creates + installs
    test.skip(!(await engineUp(page.request)), 'Docker engine unavailable')
    await deleteAllProjects(page.request)
    try {
      // --- seed two projects ---
      const a = await createProject(page.request)
      const b = await createProject(page.request)
      await page.goto('/')
      ...
      // both start checked; scope the run to just project A
      ...
      const c = await createProject(page.request)
      ...
      // scope the run to just project B
      for (const [i, id] of (await projectOrder(page.request)).entries()) {
