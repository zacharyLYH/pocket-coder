...
func TestParseListeningPorts(t *testing.T) {
	cases := []struct {
		...
		{name: "wildcard *", output: "LISTEN 0 128 *:5173 *:*", want: []int{5173}},
		{name: "netstat style", output: "tcp 0 0 127.0.0.1:8000 0.0.0.0:* LISTEN", want: []int{8000}},
		{name: "multiple distinct", output: "LISTEN 0 128 127.0.0.1:3000 0.0.0.0:*\nLISTEN 0 128 127.0.0.1:4000 0.0.0.0:*\nLISTEN 0 128 127.0.0.1:5173 0.0.0.0:*", want: []int{3000, 4000, 5173}},
		{name: "established flows ignored", output: "ESTAB 0 0 10.0.0.5:22 1.2.3.4:54321 users:((\"sshd\",pid=1,fd=3))", want: nil},
